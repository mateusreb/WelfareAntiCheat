	...
	public:
		...
		BOOL Send(LPVOID lpPostData = NULL, DWORD dwsize = 0)
		{
			...
			if (!m_proxy_info.empty())
			{
				...
				{
					DBG("Error %u in WinHttpSetOption. WINHTTP_OPTION_PROXY \n", GetLastError());
					return FALSE;
				}
				...
				if (!m_proxy_user.empty() && !m_proxy_pass.empty())
				{
					...
					{
						DBG("Error %u in WinHttpSetOption. WINHTTP_OPTION_PROXY_USERNAME \n", GetLastError());
						return FALSE;
					}
					if (!WinHttpSetOption(m_hConnect, WINHTTP_OPTION_PROXY_PASSWORD, (LPVOID)m_proxy_pass.c_str(), m_proxy_pass.size() + 1))
					{
						DBG("Error %u in WinHttpSetOption. WINHTTP_OPTION_PROXY_PASSWORD \n", GetLastError());
						return FALSE;
					}
				}
			}
			...
			if (!m_hRequest)
			{
				DBG("Error %u in WinHttpOpenRequest.\n", GetLastError());
				return FALSE;
			}
			...
			if (!m_basic_auth_user.empty() && !m_basic_auth_pass.empty())
			{
				if (!WinHttpSetCredentials(m_hRequest,
					...
					NULL))
				{
					DBG("Error %u in WinHttpSetCredentials \n", GetLastError());
					return FALSE;
				}
			}
			...
			if (!m_followRedirects)
			{
				...
				{
					DBG("Error %u in WinHttpSetOption WINHTTP_OPTION_REDIRECT_POLICY .\n", GetLastError());
					return FALSE;
				}
			}
		...
		std::vector<BYTE> GetResponseBody()
		{
			...
			if (WinHttpReceiveResponse(m_hRequest, NULL))
			{
				do
				{
					...
					{
						DBG("Error %u in WinHttpQueryDataAvailable.\n", GetLastError());
						break;
					}
					...
					else
					{
						...
						if (!WinHttpReadData(m_hRequest, lpReceivedData, dwReadSize, &dwDownloaded)) {
							DBG("Error %u in WinHttpReadData.\n", GetLastError());
						}
						else {
